...
export default function UpdateProductRow({ open = true, onClose, productRow }) {
    ...
    const handleQuantiteChange = (value) => {
        ...
        if (value - rows.find(r => r.id === productRow.id).quantite > qDispo) {
            setErrors(err => ({ ...err, quantite: 'Vous ne pouvez vendre une quantité supérieur au stock disponible' }))
        } else {
            setErrors(err => ({ ...err, quantite: null }));
        }
    ...
    useEffect(() => {
        const produit = produits.find(p => p.id === productRow.idProduit);
        const quantite = rows.find(r => r.id === productRow.id).quantite;
        if (produit && depot) {
            setLoadingData(true);
            axios
                ...
                .then(res => {
                    ...
                    const q = d.entrees.reduce((a, b) => a + b.solde, 0);
                    const qD = convertQuantity(unit, unit, unit.type_unite === 'extra' ? q * unit.equivalent : q) - quantite;
                    setUnits(d.units);
                    setUnit(unit);
                    ...
                    if (produit) {
                        const { prixTotal, prixTotalMin, entreeSortie } = fifo(d.entrees, quantite, unit, unit);
                        setTotal(formatNumber(prixTotal, 2, '.'));
                        setTotalMin(prixTotalMin);
                        setPvUnitaire(formatNumber(prixTotal / quantite));
                        setEntreesSorties(entreeSortie);
                    }
                })
            ...
            <StyledContainer>
                <StyledContentContainer>
                    <div className="header">
                        <Typography variant="caption">Modification d'une ligne d'articles</Typography>
                        <div>
